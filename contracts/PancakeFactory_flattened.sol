...
contract PancakePair is PancakeERC20 {
    ...
    function _update(
        ...
        require(
            balance0 <= uint112(2**112 - 1) && balance1 <= uint112(2**112 - 1),
            "Pancake: OVERFLOW"
        );
...
contract PancakeFactory is IPancakeFactory {
    ...
    event PairCreatedv2(address indexed token0, address indexed token1, address pair, uint);
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        ...
        allPairs.push(pair);
        emit PairCreatedv2(token0, token1, pair, allPairs.length);
    }
